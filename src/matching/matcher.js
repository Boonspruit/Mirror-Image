...
function hasDetectedHand(userFeatures) {
  return Number.isFinite(userFeatures?.handPresent) && userFeatures.handPresent >= HAND_SIGNAL_THRESHOLD
}
...
export function rankMemes(userFeatures, memes, weights = DEFAULT_MATCH_WEIGHTS) {
  ...
  return memes.map((meme) => {
    ...
    const handModeActive = hasDetectedHand(userFeatures)
    const gestureMatched = matchesRequiredHandGesture(userFeatures, meme.handFeatures)
    // Hand-aware memes participate only while a hand is actually present.
    const comparison = meme.handFeatures && !handModeActive
      ? null
      : compareExpressions(userFeatures, memeFeatures, applicableWeights)
    return {
      meme,
      comparison,
      priority: gestureMatched ? 2 : (meme.handFeatures && handModeActive ? 1 : 0),
