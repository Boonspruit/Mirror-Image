...
test('focused navigation preserves one camera session and trains beside a live preview', async ({ page }) => {
  ...
  await page.getByRole('button', {name:'Inspect Alma Worried Hamster',exact:true}).click()
  ...
  await expect(dialog).toContainText('Alma Worried Hamster now matches this expression.')
